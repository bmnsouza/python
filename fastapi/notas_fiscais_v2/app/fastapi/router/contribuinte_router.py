...
from app.utils.http_utils import to_http_exception
...
async def get_list(
    request: Request, response: Response, asc: Optional[str] = Query(None), des: Optional[str] = Query(None), offset: int = Query(0, ge=0),
    limit: int = Query(50, ge=1), fields: Optional[str] = Query(None), accept_ranges: Optional[int] = Query(None), session: AsyncSession = Depends(get_session)
):
    # Filtros dinâmicos
...
async def get_by_cd(cd_contribuinte: str, session: AsyncSession = Depends(get_session)):
    try:
        service = ContribuinteService(session)
