...
from .data_loader import BenchmarkData
...
def _build_context(data: BenchmarkData, users: int, spawn_rate: int, run_time: str) -> dict:
    ...
    payload_data = [round(v / 1024, 2) for v in stats["Average Content Size"].tolist()]
    ...
    return {
        ...
        "kpi_rps_rows": _kpi_ep_rows(
            stats,
            "Requests/s",
            lambda v: f"{v:.2f} req/s",
        ),
        "kpi_failures_rows": _kpi_ep_rows(stats, "Failure Count", int),
        "kpi_latency_rows": _kpi_ep_rows(
            stats,
            "Min Response Time",
            lambda v: f"{round(v):,} ms",
        ),
        ...
        "payload_data_js": str(payload_data),
    }
...
def _failures_section(failures: pd.DataFrame) -> str:
    ...
    rows = "".join(
        ...
        f"<td>{int(row.get('Occurrences', 0))}</td>"
        f"</tr>"
...
def _payload_section(benchmark: pd.DataFrame | None) -> str:
    ...
    rows = "".join(
        f"<tr><td>{row['resolver']}</td><td>{row['payload_medio']:.2f} KB</td>"
    ...
    <html lang="pt-BR">
      <head>
        ...
        <style>*{{box-sizing:border-box;margin:0;padding:0}}
          ...
          section{{margin-bottom:32px}} .section-label{{font-size:11px;font-weight:600;letter-spacing:.08em;
          ...
          tr:last-child td{{border-bottom:none}} .pill{{padding:2px 9px;border-radius:20px;font-size:11px;
          font-weight:500}}
          .card{{background:var(--surface);border:1px solid var(--border);border-radius:10px;overflow:hidden}}
          .card .scroll{{overflow-x:auto}}
          footer{{font-size:11px;color:var(--hint);border-top:1px solid var(--border);padding-top:16px;display:flex;
      ...
      <body>
        <h1>📊 Relatório de Benchmark End-to-End</h1>
        <p class="subtitle">Locust headless · POST</p>
        <section>
          <p class="section-label">⚡ Visão geral</p>
          <div class="kpi-grid">
            <div class="kpi">
              <em>Total de requisições</em><strong>{c['total_requests']}</strong>
              <ul class="kpi-ep-list">
                {c['kpi_requests_rows']}
              </ul>
            </div>
            <div class="kpi">
