...
async def get_contribuintes(page: int = DEFAULT_PAGE, session: AsyncSession = Depends(get_session)):
    try:
...
async def get_contribuintes_danfe_endereco(filtro_nome: str, page: int = DEFAULT_PAGE, session: AsyncSession = Depends(get_session)):
    try:
...
async def create_contribuinte(contribuinte: ContribuinteCreate, session: AsyncSession = Depends(get_session)):
    try:
        return await contribuinte_service.create_contribuinte(contribuinte=contribuinte, session=session)
...
async def update_contribuinte(cd_contribuinte: str, contribuinte: ContribuinteUpdate, session: AsyncSession = Depends(get_session)):
    try:
        result = await contribuinte_service.update_contribuinte(cd_contribuinte=cd_contribuinte, contribuinte=contribuinte, session=session)
        if not result["data"]:
            raise HTTPException(status_code=404, detail="Contribuinte não encontrado")
        return result
    except DatabaseError as e:
...
async def delete_contribuinte(cd_contribuinte: str, session: AsyncSession = Depends(get_session)):
    try:
        result = await contribuinte_service.delete_contribuinte(cd_contribuinte=cd_contribuinte, session=session)
        if not result["data"]:
            raise HTTPException(status_code=404, detail="Contribuinte não encontrado")
